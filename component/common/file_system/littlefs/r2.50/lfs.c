...
static lfs_stag_t lfs_dir_fetchmatch(lfs_t *lfs,
    ...
    uint32_t revs[2] = {0, 0};
    uint32_t fmt_flag = 0;    // realtek: blank media => return LFS_ERR_NOTFMT
    int r = 0;
    for (int i = 0; i < 2; i++) {
        int err = lfs_bd_read(lfs,
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            if (!lfs_tag_isvalid(tag)) {
                ...
                // realtek: tag 全 0xff 说明该 block 尚未写过，整盘很可能未格式化
                if (tag == 0xffffffff) {
                    fmt_flag = 1;
                }
                break;
    ...
    // realtek: 整段扫描结束仍未能定位有效目录，且看到的是全 ff 的擦除态，
    // 视为未格式化而非真损坏，让上层走自动 format 路径
    if (fmt_flag) {
