...
int httpc_request_write_header_start(struct httpc_conn *conn, char *method, char *resource, char *content_type, size_t content_len)
{
	...
	if (conn->request_header) {
		...
		snprintf((char *)conn->request_header, header_len + 1, "%s ", method);
		strcat((char *)conn->request_header, resource);

		if ((conn->port == 80) || (conn->port == 443)) {
			snprintf((char *)conn->request_header + strlen((char *)conn->request_header), header_len + 1 - strlen((char *)conn->request_header), " HTTP/1.%d%sHost: %s%s",
					 httpc_http_1_0_request_used ? 0 : 1, HTTP_CRLF, conn->host, HTTP_CRLF);
		} else {
			snprintf((char *)conn->request_header + strlen((char *)conn->request_header), header_len + 1 - strlen((char *)conn->request_header), " HTTP/1.%d%sHost: %s:%d%s",
					 httpc_http_1_0_request_used ? 0 : 1, HTTP_CRLF, conn->host, conn->port, HTTP_CRLF);
		}

		if (content_type) {
			snprintf((char *)conn->request_header + strlen((char *)conn->request_header), header_len + 1 - strlen((char *)conn->request_header), "Content-Type: %s%s", content_type, HTTP_CRLF);
		}

		if (content_len) {
			snprintf((char *)conn->request_header + strlen((char *)conn->request_header), header_len + 1 - strlen((char *)conn->request_header), "Content-Length: %d%s", content_len, HTTP_CRLF);
		}

		if (conn->user_password) {
			snprintf((char *)conn->request_header + strlen((char *)conn->request_header), header_len + 1 - strlen((char *)conn->request_header), "Authorization: Basic %s%s", conn->user_password,
					 HTTP_CRLF);
		}
	} else {
...
int httpc_response_read_header(struct httpc_conn *conn)
{
	...
	if (header_existed) {
		...
		if (conn->response.header) {
			...
			while (ptr < (conn->response.header + conn->response.header_len)) {
				if ((*ptr == '\r') && (*(ptr + 1) == '\n')) {
					conn->response.status_len = ptr - (char *)conn->response.status;
					break;
				}
			...
			if (memcmp(conn->response.status, status_301, strlen(status_301)) == 0 ||
				...
				if (strstr(ptr, "Location:")) {
					...
					while (ptr < (conn->response.header + conn->response.header_len)) {
						if ((*ptr == '\r') && (*(ptr + 1) == '\n')) {
							conn->response.location_len = ptr - (char *)conn->response.location;
							break;
						}
			...
			if (strstr(ptr, "Content-Type:")) {
				...
				while (ptr < (conn->response.header + conn->response.header_len)) {
					if (*ptr != ' ') {
						conn->response.content_type = (uint8_t *)ptr;
						break;
					}
				...
				ptr = (char *)conn->response.content_type + 1;

				while (ptr < (conn->response.header + conn->response.header_len)) {
					if ((*ptr == '\r') && (*(ptr + 1) == '\n')) {
						conn->response.content_type_len = ptr - (char *)conn->response.content_type;
						break;
					}
			...
			if (strstr(ptr, "Transfer-Encoding:")) {
				...
				while (ptr < (conn->response.header + conn->response.header_len)) {
					if (*ptr != ' ') {
						conn->response.trans_enc = (uint8_t *)ptr;
						break;
					}
				...
				while (ptr < (conn->response.header + conn->response.header_len)) {
					if ((*ptr == '\r') && (*(ptr + 1) == '\n')) {
						conn->response.trans_enc_len = ptr - (char *)conn->response.trans_enc;
						break;
					}
